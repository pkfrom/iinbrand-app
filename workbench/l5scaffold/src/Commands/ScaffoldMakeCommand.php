...
namespace Fromz\L5scaffold\Commands;

use Fromz\L5scaffold\Makes\MakeRoute;
use Illuminate\Console\AppNamespaceDetectorTrait;
use Illuminate\Console\Command;
use Illuminate\Filesystem\Filesystem;
use Illuminate\Foundation\Composer;
use Fromz\L5scaffold\Makes\MakeController;
use Fromz\L5scaffold\Makes\MakeLayout;
use Fromz\L5scaffold\Makes\MakeMigration;
use Fromz\L5scaffold\Makes\MakeModel;
use Fromz\L5scaffold\Makes\MakerTrait;
use Fromz\L5scaffold\Makes\MakeSeed;
use Fromz\L5scaffold\Makes\MakeView;
...
class ScaffoldMakeCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Create a scaffold with AdminLTE';
    ...
    {
        $routeContent = 'Route::resource("'.$this->getObjName("names").'","'.$this->getObjName("Name").'Controller");';
        $this->info('Add '.$this->getObjName("names").' in routes.php...');

        $this->info('======================================================');
        $this->info('Goto '.config('app.url').'/'.$this->getObjName("names"));
        $this->info('And don\'t forget to run: php artisan migrate');
        new MakeRoute($this, $this->files , $routeContent);
